...
rvalue и lvalue- это характеристика выражения

lvalue - l=location(имеет положение, адрес даже если временный)
```cpp
int & foo();
foo()=5; //ok for c++
```
Выражения обладающие идентичность - `gvalue (generalized value)` местом в памяти
